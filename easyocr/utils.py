...
def compute_ratio_and_resize(img, width, height, model_height):
    ...
    new_width, new_height = (
        ...
        if ratio < 1.0
        else (int(model_height * ratio), model_height)
    )
    ...
    if isinstance(img, np.ndarray):
        ...
        img = TF.resize(
            img.unsqueeze(0), (new_height, new_width), antialias=True
        )  # Note: TF.resize expects (C,H,W)
        img.squeeze_(0)
    else:
    ...
    #         img = TF.resize(img.squeeze(0),(model_height,int(model_height*ratio)), antialias=True)
    #         img.unsqueeze_(0)
    # else:
    #     if type(img) == np.array:
    #         img = cv2.resize(img,(int(model_height*ratio),model_height),interpolation=Image.Resampling.LANCZOS)
    #     elif type(img) == torch.Tensor:
    #         img = TF.resize(img.squeeze(0),(int(model_height*ratio),model_height), antialias=True)
    #         img.unsqueeze_(0)
    # return img,ratio
...
def reformat_input(image):
    if type(image) == str:
        if image.startswith("http://") or image.startswith("https://"):
            ...
            img_grey = cv2.imread(tmp, cv2.IMREAD_GRAYSCALE)
            os.remove(tmp)
        else:
    ...
    # wlp:
        ...
        if len(image.shape) == 2:  # grayscale
            img_grey = image  # HxW
            img = img_grey.unsqueeze(-1).repeat(1, 1, 3)  # HxWxC
        elif len(image.shape) == 3 and image.shape[2] == 1:
            img_grey = image.squeeze()
            img = img_grey.unsqueeze(-1).repeat(1, 1, 3)  # HxWxC
        elif len(image.shape) == 3 and image.shape[2] == 3:  # #HxWxC RGBscale
            img = image
            img_grey = TF.rgb_to_grayscale(
                image.permute(2, 0, 1), num_output_channels=1
            )
            img_grey = img_grey.permute(1, 2, 0).squeeze()
        # batch case
        # TODO: checke these and implement the other batch cases
        elif len(image.shape) == 4 and image.shape[1] == 3:  # BxCxHxW grayscale
            img_grey = image
            img = img_grey.repeat(1, 3, 1, 1)
        elif len(image.shape) == 4 and image.shape[1] == 3:  # BxCxHxW RGBscale
            img = image
            img_grey = TF.rgb_to_grayscale(image, num_output_channels=1)

    else:
        raise ValueError(
            "Invalid input type. Supporting format = string(file path or url), bytes, numpy array, torch tensor"
        )
